...
(function () {
  ...
  const HAS_INTERSECTION = 'IntersectionObserver' in window;
  const io = HAS_INTERSECTION ? new IntersectionObserver((entries) => {
    entries.forEach(en => {
    ...
    const mo = new MutationObserver((muts) => {
      for (const m of muts) {
        m.addedNodes.forEach(n => {
          ...
          markTiltables(n);
          observeReveals(n);
          if (n.matches && n.matches(REVEAL_SEL)) {
            n.classList.add('f3d-reveal');
            if (io) io.observe(n);
            else n.classList.add('f3d-in');
          }
